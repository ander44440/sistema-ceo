...
> **Versão:** 1.1 — 12/08/2026  
> **Capacidade:** CAP-04 — Gestão do Conhecimento
> **Implementação:** IMP-070-B1 **Homologado / Encerrado** — Baseline; clarificação CA-070-5/6 12/08/2026.
...
| **CA-070-5** | **LACUNA EXPLÍCITA** do Acervo indica ausência de conhecimento oficial persistente; **não** constitui, por si só, lacuna material de decisão (REQ-048 `solicitar_dados`) nem bloqueia decisão executiva; **não** obriga `monitorar` só por Acervo vazio. | Acervo vazio + factos suficientes no contexto corrente + pedido de decisão: a deliberação pode fechar sem ser forçada a `solicitar_dados`/`monitorar` *apenas* pela LACUNA EXPLÍCITA; a LACUNA continua declarada. Sim = pass. |
| **CA-070-6** | Quando houver factos suficientes fornecidos no contexto corrente (mensagem/fio), o CEO pode deliberar sobre esses factos **sem** os converter em património oficial do Acervo. | Resposta que usa factos da conversa não os apresenta como item/KNW do Acervo sem registo/aprovação. Sim = pass. |

## Distinção obrigatória

| Conceito | Norma | Significado |
|----------|-------|-------------|
| **LACUNA EXPLÍCITA** (Acervo / Porta) | Este REQ; ARQ-031 D1; REQ-072 | Sem item apto no Acervo para o âmbito — declarar; não inventar património oficial |
| **Lacuna material de decisão** (`lacunas[]` / `solicitar_dados`) | REQ-048; REQ-049 | Falta informação **essencial à decisão** no contexto deliberativo |
...
* Política de fecho sob pedido explícito de decisão — REQ-048 / política de decisão sob conflito (não substituída por este REQ).

## Dependências

* REQ-004 (registo estruturado); ARQ-006; ARQ-007; ARQ-031 D1; CNC-002.
* Distinção decisória: REQ-048; REQ-049.
...
* Ambiguidade entre facto de conversa e item oficial — mitigação: só o índice define pertença (CA-070-1); CA-070-5/6 separam lacuna de Acervo de lacuna material.
...
| Decisões derivadas | Clarificação CA-070-5/6 — peso decisório da LACUNA EXPLÍCITA (12/08/2026) |
| Implementação | **IMP-070** (Baseline) + hint estágio 6 (`hintEstagio6LacunaFonteOficial`) |
| Testes | `app/src/mre/p1-lacuna-acervo-vs-material.test.js`; `fonteOficial.test.js` |
...
| 1.1 | 12/08/2026 | Engenheiro (Cursor) | CA-070-5/6 + distinção Acervo vs lacuna material | LACUNA EXPLÍCITA sobreponderada como bloqueio decisório (caso RotaSul) | **Aprovado** (clarificação normativa; Baseline CAP-04 preservada) |
